...
public class ProjectDao extends DaoBase {
	...
	/**
	 * This method uses JDBC methods to retrieve all project rows from the project
	 * table. It does not retrieve any materials, steps, or categories. The project
	 * rows are ordered by project name.
	 * 
	...
	/**
	 * This method used JDBC method calls to retrieve a single project row, along
	 * with its associated materials, steps, and categories.
	 * 
	...
	public Optional<Project> fetchProjectById(Integer projectId) {
		...
		try (Connection conn = DbConnection.getConnection()) {
			...
			/*
			 * This try block is used to wrap all code to return the project row and
			 * accompanying materials, steps and categories so that, if an error occurs at
			 * any place, the transaction can be rolled back correctly.
			 */
			try {
				...
				try (PreparedStatement stmt = conn.prepareStatement(sql)) {
					...
					/*
					 * Alternate approach. If you know your parameter will never be null you can set
					 * the parameter on the statement directly using JDBC.
					 */
				...
				/*
				 * This null check isn't expressly needed because if the project ID is invalid,
				 * each method will simply return an empty list. However, it avoids three
				 ...
				 * the given project ID is invalid.
				 */
	...
	/**
	 * This method uses JDBC method calls to retrieve project materials for the
	 * given project ID. The connection is supplied by the caller so that project
	 * materials can be retrieved on the current transaction.
	 * 
	 * @param conn      The caller-supplied connection.
	...
	public boolean modifyProjectDetails(Project project) {
		...
		// Call executeUpdate and check if the return value is 1 and save result in modified variable. Commit transaction
	...
	public boolean deleteProject(Integer projectId) {
		...
		// set placeholder for projectId in SQL
		String sql = ""
		...
		// Set parameters on the PreparedStatement	
			try (PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				/*
				 * commit or rollback transaction if error. if single row is deleted and returns
				 * 1 if not it was not successful rollback transaction.
				 */		
				commitTransaction(conn);
				return deleted;
			} catch (Exception e) {
